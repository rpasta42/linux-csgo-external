...
int main() {
    ...
    int kkKeycodeShoot = XKeysymToKeycode(display, XK_I);
    ...
    XGrabKey(display, kkKeycodeShoot, modifiers, root, false,
       ...
       usleep(100);
	    ...
	    while (XPending(display) > 0) {
			XNextEvent(display, &ev);
                if (ev.type == KeyPress){
               if (ev.xkey.keycode == kkKeycodeShoot) { //f5
                  csgo.kkShootKeyDown = !csgo.kkShootKeyDown;
                  cout << "shoot" << endl;
               }
    ...
    XUngrabKey(display, keycodeBHopEnable, modifiers, root);
    XUngrabKey(display, kkKeycodeShoot, modifiers, root);
